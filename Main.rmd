...
full_model_compiled <- readRDS("output/data/Single_species_compiled_data_gar2in_Canham_3_sampling.rds")
size_est_data <- full_model_compiled$measurement_data %>%
  mutate(S_hat_6obs = S_hat) %>% 
  group_by(treeid_factor) %>%
  ungroup() %>%
  select(treeid_factor, S_obs, S_hat_6obs, census, time)
  #5 obs individual-level model pars

  #6 individual-level model pars
  ...
  #Compare size estimates from 6 obs ind model, 5 obs ind model, 5 obs species model

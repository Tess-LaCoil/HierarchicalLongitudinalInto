...
To test the predictive power of the model we run a second fit using the first five observations for each individual and compare the results.
```{r}
fit_5_obs <- TRUE
...
if(fit_5_obs){
  ...
  filename <- paste(single_species_controls$fit_save_path, "GRecondita_5obs_", sep="_")
  filename <- paste(filename, ".rds", sep="")
  saveRDS(fit, file=filename) #Output to file
  ...
  fit <- fit[[3]] #get rid of other elements in list
  saveRDS(fit, object="output/data/Single_Species_Fit_Canham_GRecondita_5obs.rds")
  rm(fit)
  ...
  if(plot_diagnostics_species_level){
    plot_diagnostic_trace(species_only_fit, 3, c("species_max_growth", 
                                                 "species_diameter_at_max_growth", 
                                                 "species_K"), 
                          "GRecondita_5obs_Canham_species_model_pars", FALSE)
  }
  
  #Save fit
saveRDS(species_only_fit, "output/data/GRecondita_SpeciesLevelFit_5obs.rds")
  ...
  #5 obs individual-level model pars
  fit <- readRDS("output/data/Single_Species_Fit_Canham_GRecondita_5obs.rds")
  dataset <- readRDS("data/gar2in_5_rstan_data.rds")
  #Extract samples
  est_data <- rstan::extract(fit, permuted=TRUE)
  #Building data frames
  ...
  #Construct estimates of size 6 based on size 5 and individual parameters
  size_data_predsize <- size_est_data %>%
    group_by(treeid_factor) %>%
    mutate(S_0_obs = first(S_obs)) %>%
    filter(census == 6) %>%
    mutate(S_hat_final_size_5obs = NA, #Initialise variables to get estimated final sizes
           S_hat_final_size_species_model = NA,
           S_hat_final_size_species_model_from_S_0_obs = NA)
  penultimate_size <- measurement_data %>%
    filter(census == 5)
  
  size_data_predsize$penultimate_size <- penultimate_size$S_hat
  ...
  for(i in 1:nrow(size_data_predsize)){
    ...
    #Estimation from species model based on first observed size
    temp_size_est <- rk4_est(S_0 = size_data_predsize$penultimate_size[i], 
            growth = growth_function, 
            pars = species_growth_pars, 
  ...
  print(paste("RMSE for species average model extrapolated from observed 1st size",
    sqrt(sum(size_data_predsize$S_hat_final_size_species_model_from_S_0_obs)/nrow(size_data_predsize))))
}

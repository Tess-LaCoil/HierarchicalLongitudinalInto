...
We are going to build 2 more models: the hierarachical model with Canham growth function fit to 5 observations per individual, and a hierarchical model with constant growth function and individual level average growth parameters fit to 5 observations. The constant growth model is directly analogous to a linear mixed effects model for sizes over time with individual effect term. 
...
if(extract_predictive_RMSE){
  ...
  #Calculate error
  starting_size$S_final_error <- starting_size$S_6_obs - starting_size$S_final_hat
  par(mfrow = c(3,1))
  #Histogram of errors
  for(i in 1:length(fit_name)){

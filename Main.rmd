...
species_growth_pars <- c(
  ...
  S_0 = 1,
  S_final = 25, #Get maximum size
  source = "Species-level model"
)

#Import individual-level parameters
individual_data <- readRDS("output/data/Single_species_compiled_data_gar2in_Canham_3_sampling.rds")$individual_data
...
ind_growth_pars <- tibble(
  ...
  k = individual_data$canham_K_hat,
  S_0 = individual_data$canham_S_0_hat,
  S_final = 25, #Get vector of final 
  source = "Individual fits"
)

median_individual_pars <- c(
  g_max = median(ind_growth_pars$g_max),
  S_max = median(ind_growth_pars$S_max),
  k = median(ind_growth_pars$k),
  S_0 = 1,
  S_final = 25,
  source = "Individual median pars"
)

plot_data <- rbind(ind_growth_pars, median_individual_pars, species_growth_pars)
